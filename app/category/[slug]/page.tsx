			...
			<div className='w-full my-10'>
				<div className='container px-3 mx-auto'>
					<div className='flex items-center text-sm gap-x-2.5'>
						...
						<span>/</span>
						<Link href='/category' className='text-[#019a04]'>
							Category
						</Link>
						<span>/</span>
					...
					<div className='my-5'>
						...
						{category?.mota && <p className='text-gray-700 mb-6 text-lg'>{category.mota}</p>}
					</div>
				</div>
				<div className='container px-3 mx-auto'>
						...
						<div className='col-span-4 flex flex-wrap gap-5 max-[500px]:col-span-5'>
							{data.stores && data.stores.length > 0 ? (
								data.stores.map(store => {
									const storeImg = resolveImageUrl(store.image, { fallback: '/store/1.jpg' });
									const storeImgExt =
										storeImg.startsWith('http://') || storeImg.startsWith('https://');
									return (
										<div
											key={store._id}
											className='w-[calc(50%-10px)] max-h-48 bg-white border border-gray-100 shadow rounded p-3 max-[500px]:w-full'
										>
