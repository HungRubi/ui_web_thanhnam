...
const ModalCoupon: React.FC<ModalCouponProps> = ({ btn, offers = [], store, selectedOfferIndex }) => {
	...
	const modalLogoSrc = resolveGlobalMediaUrl(data?.logo) ?? '/images/logo.jpg';
	const modalLogoRemote = modalLogoSrc.startsWith('http://') || modalLogoSrc.startsWith('https://');

	const handleOpen = () => {
		// pick random offer when opening or use selected offer index if provided
		if (offers && offers.length > 0) {
			const idx =
				selectedOfferIndex !== undefined ? selectedOfferIndex : Math.floor(Math.random() * offers.length);
	...
	return (
		<>
			...
			{btn === 1 && (
				<div className='w-[235px] flex-none flex justify-center'>
					...
					>
						...
						<div
							className='absolute text-white transition-all rounded-md get_code 
			...
			{btn === 2 && (
				<button className='text-[#019a04] cursor-pointer' onClick={handleOpen}>
					{store.tenstore}
				</button>
			)}
			{isOpen && (
