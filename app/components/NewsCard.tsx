...
const NewsCard: FC<NewsCardProps> = ({ title, img, subTitle, link, isAuthor, className, isSub, formatDate }) => {
	...
	return (
		<div
			className={`border border-gray-200 rounded-lg px-3 bg-white shadow-sm hover:shadow-md transition 
      w-[calc(100%/5-8px)] relative ${className} max-[450px]:w-full  max-[650px]:w-[calc(100%/2-7px)] max-[900px]:w-[calc(100%/3-7px)]`}
		>
			<Link href={link} className='w-full h-[180px] flex items-center justify-center mb-3'>
				<Image
					src={imageSrc}
					alt={title}
					width={300}
			...
			<Link href={link} className='font-[560] text-gray-800 text-[15px] leading-tight line-clamp-3 mb-2'>
				{title}
			</Link>
			<p className={`font-normal text-gray-600 text-sm leading-5 line-clamp-3 mb-2 ${isSub}`}>{subTitle}</p>
			<div
				className={`w-9/10 mx-auto mt-5 border-t border-gray-300 ${isAuthor ? 'hidden' : 'block'} ${isSub}`}
			></div>
			<Link
				href={link}
