...
export default function EventDetail() {
  ...
  if (eventsLoading) {
    return (
      <>
        <Header />
        <div className="w-full min-h-[60vh] flex items-center justify-center"><p className="text-gray-500">Đang tải event...</p></div>
        <Footer />
      </>
    );
  }
  ...
  if (eventsError || !currentEvent) {
    return (
      <>
        <Header />
        <div className="w-full min-h-[60vh] flex items-center justify-center"><p className="text-red-500">{eventsError || "Không tìm thấy event"}</p></div>
        <Footer />
      </>
    );
  }
      ...
      <div className="w-full mt-10 mb-7">
        <div className="container px-3 mx-auto grid grid-cols-1 md:grid-cols-5 gap-5">
          <div className="col-span-4 bg-white shadow p-4">
            ...
            <div className="my-5 text-2xl text-gray-700 font-semibold">{currentEvent.tendanhmuc} Coupons</div>
            {currentEvent.mota ? (
              <p className="leading-7 text-gray-700">{currentEvent.mota}</p>
            ) : (
              <p className="leading-7 text-gray-700">Thông tin về event đang được cập nhật. Vui lòng quay lại sau.</p>
            )}
            <div className="min-h-[300px] flex items-center justify-center flex-col mt-6">
              <Image width={800} height={600} src={'/images/404.gif'} alt={currentEvent.tendanhmuc} className="max-w-[80%] w-100" />
