				...
				<div className='container px-3 mx-auto mt-12 grid grid-cols-4 sm:grid-cols-4 gap-x-7'>
					...
					<div className='col-span-3 max-[435px]:mx-auto max-[435px]:w-full max-[435px]:col-span-full'>
						...
						<div className='mb-5 w-full flex items-center gap-x-2.5'>
							<div className='px-6 py-2 rounded-md font-bold shadow cursor-pointer bg-[#019a04] text-white transition'>
								All ({offers.length})
							</div>
						</div>
						...
						{offers && offers.length > 0 ? (
							offers.map((o: any, i: number) => (
								<div
									key={o._id || i}
									className='w-full bg-white flex p-2.5 min-h-[130px] max-[450px]:flex-col items-center mb-4 shadow rounded'
								>
									<div className='flex items-center'>
										...
										<div className='w-full flex flex-col justify-start h-[130px] ml-2.5'>
											<p className='text-[#019a04] font-bold  truncate'>
												{o.verified === 'Yes' ? 'Verified Code' : 'Deal'}
											</p>
						...
						{store.about && (
							<>
								...
								<div className='w-full border border-gray-200 p-4 text-[#555]'>
									<div className='mb-4 leading-7' dangerouslySetInnerHTML={{ __html: store.about }} />
								</div>
							</>
						)}
						...
						{formattedFaqs && (
							<>
								...
								<div className='w-full border border-gray-200 p-4 text-[#555]'>
									<div className='leading-7' dangerouslySetInnerHTML={{ __html: formattedFaqs }} />
								</div>
							</>
						)}
					</div>
					...
					<div className='w-full grid grid-cols-1 sm:grid-cols-3 text-[#019a04]'>
						<div className='col-span-1 flex flex-col'>
							{apiStores?.slice(0, 5).map((item: any) => (
								<Link href={`/store/${item.slug}`} key={item._id}>
									{item.tenstore}
								</Link>
							))}
						</div>
