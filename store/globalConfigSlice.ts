...
export const getGlobalConfig = createAsyncThunk('globalConfig/fetch', async (_, { rejectWithValue }) => {
	try {
		const data = await fetchGlobalConfig();
		return data;
	} catch (error) {
		return rejectWithValue(error instanceof Error ? error.message : 'Failed to fetch global config');
	}
});
